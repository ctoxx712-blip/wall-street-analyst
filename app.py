...
bull/base/bear. Emit raw JSON only, no comments. Supported shapes:
...
LANGUAGES = {
    ...
    "both": "Reply BILINGUALLY. For every heading, paragraph and bullet, give the "
            "English first, then the Traditional Chinese (Hong Kong style) immediately "
            "beneath it. In tables, write each header as 'English / 中文' in "
            "the same cell rather than duplicating the table. Keep standard finance "
            "terms in English.",
}
...
def chat():
    ...
    system += "\n\nLANGUAGE. " + LANGUAGES.get(data.get("lang", "en"), LANGUAGES["en"])
